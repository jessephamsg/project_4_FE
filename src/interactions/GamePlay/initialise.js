...
export default {
    ...
    getLatestLocalGameState(gameConfig, gameName, kidName) {
        ...
        let currentLevel = DEFAULT_CURRENT_LEVEL,
            currentOption = DEFAULT_CURRENT_OPTION;
        if (this.isFirstTimePlayingGame(gameName, kidName) === false)({
            currentLevel,
            currentOption
        } = LocalGameState.getGameLocal()[kidName][gameName])
        return {
            totalLevel,
            currentLevel,
            currentOption
